...
## Fix 6: Auto-Fill Correct Column When Creating Person

When a user creates a new person, the system must:
1. Detect the script of the entered `PrimaryName`
2. Also fill the corresponding language column (`NameArabic`, `NameEnglish`, or `NameNobiin`)

### File to Modify

`Services/PersonService.cs` (or wherever person creation is handled)

### Find the Create/Add Person Method

Look for a method like `CreatePersonAsync`, `AddPersonAsync`, or similar that creates a new person.

### Add This Logic Before Saving

```csharp
// When creating a person, auto-fill the correct language column based on PrimaryName script
private void AutoFillLanguageColumn(Person person)
{
    ...
    {
        case "Arabic":
            // If NameArabic is not already set, copy from PrimaryName
            if (string.IsNullOrWhiteSpace(person.NameArabic))
            {
        ...
        case "English":
            // If NameEnglish is not already set, copy from PrimaryName
            if (string.IsNullOrWhiteSpace(person.NameEnglish))
            {
        ...
        case "Nobiin":
            // If NameNobiin is not already set, copy from PrimaryName
            if (string.IsNullOrWhiteSpace(person.NameNobiin))
            {
...
### Update Create Person Method

Find the method that creates a person and add the call:

```csharp
public async Task<Person> CreatePersonAsync(CreatePersonRequest request, long userId)
{
    var person = new Person
    {
        Id = Guid.NewGuid(),
        OrgId = request.OrgId,
        PrimaryName = request.Name,
        Sex = request.Sex,
        // ... other properties
        CreatedAt = DateTime.UtcNow,
    ...
    // AUTO-FILL the correct language column based on script
    AutoFillLanguageColumn(person);
...
### Also Update Edit/Update Person Method

If user updates the `PrimaryName`, also update the corresponding language column:

```csharp
public async Task<Person> UpdatePersonAsync(Guid personId, UpdatePersonRequest request, long userId)
{
    var person = await _context.People.FindAsync(personId);
    if (person == null) throw new NotFoundException("Person not found");

    // If PrimaryName changed, update the correct language column
    if (!string.IsNullOrWhiteSpace(request.Name) && request.Name != person.PrimaryName)
    {
        person.PrimaryName = request.Name;
        
        // Clear old language columns and set the new one based on script
        var script = DetectScriptFromContent(request.Name);
        
        // Reset all to allow re-detection
        // Or only update the matching column - depends on your business logic
        switch (script)
        {
            case "Arabic":
                person.NameArabic = request.Name;
                break;
            case "English":
                person.NameEnglish = request.Name;
                break;
            case "Nobiin":
                person.NameNobiin = request.Name;
                break;
        }
    }
...
## Fix 7: Update GEDCOM Import to Fill Correct Column

The `GedcomService.cs` also creates persons. Update `CreatePersonFromGedcom`:

### Find in GedcomService.cs (around line 471)
...
|-----|------|-------------|
| Fix 1 | NameTransliterationService.cs | Rewrite `GenerateMissingNamesForPersonAsync` |
| Fix 2 | NameTransliterationService.cs | Add `LookupCacheBySourceColumn` helper |
| Fix 3 | NameTransliterationService.cs | Add `DetectScriptFromContent` helper |
| Fix 4 | NameTransliterationService.cs | Enforce complete cache rule in `TransliterateNameAsync` |
| Fix 5 | NameTransliterationService.cs | Update `SaveMappingAsync` to update incomplete mappings |
| Fix 6 | PersonService.cs | Auto-fill correct language column on create/update |
| Fix 7 | GedcomService.cs | Set correct language column on GEDCOM import |

**Apply all 7 fixes for complete solution.**
